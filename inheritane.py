...
class Desk(Furniture):
    ...
    def __init__(self, category, material, length, width, height, drawers, location, price, quantity):
     Furniture.__init__(self, category, material, length, width, height, price, quantity)
    ...
    def __str__(self):
        line_item = 'Product: ' + self.get_category() + ' Material: ' + self.get_material() + ' Length: ' + str(self.get_length()) \
                    ...
                    + ' Drawer location: ' + self.get_location() + ' Each: ${:0,.2f}'.format(self.get_price()) + ' Quantity: ' + str(self.get_quantity()) \
